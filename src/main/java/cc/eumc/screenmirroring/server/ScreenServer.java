...
public class ScreenServer extends Thread {
    ...
    public ScreenServer(EusScreenMirroring plugin, MirrorManager mirrorManager, int port) throws SocketException {
        this.port = port;
        this.mirrorManager = mirrorManager;
        this.socket = new DatagramSocket(port);
        this.plugin = plugin;

        this.showError = plugin.getConfig().getBoolean("General.PrintScreenServerError", false);
    }
    ...
    public void run() {
        while (running) {
            ...
            try {
                ...
                if (showError) {
                    plugin.sendWarn(String.format("Failed receiving packet: %s", e.getMessage()));
                }
                continue;
            }
...
//                    }
                ...
                if (showError) {
                    e.printStackTrace();
                    plugin.sendWarn(String.format("Illegal packet: %s", e.getMessage()));
                }
            }
        }
